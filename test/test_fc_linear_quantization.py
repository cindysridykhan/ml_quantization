...
import torch
from src.quantization.fc_linear_quantization import *
...
def sample_W_X_b():
    ...
    with open('test/test_fc_linear_quantization_data.p', 'rb') as f:
        data = pickle.load(f)
    X = data['X']
    W = data['W']
    b = data['b']
...
class TestMatMulQuantization:
    @mock.patch('src.quantization.fc_linear_quantization.get_quantized_range', return_value=(-128, 127))
